...
#define TVIRTUAL	/* virtual */   // unnecessary by tempalte metaprogramming
...
class Vec4Func {
public:
    TVIRTUAL Vec4 apply(int i) const {
        return Vec4(0, 0, 0, 0);
    }
};
...
class Func {
public:
    TVIRTUAL Vec3 apply(class Vec4Array* a, int i, const Vec3 v, const float w) const {
        return Vec3(0, 0, 0);
    }
};
...
public:
    ...
    VIRTUAL void set(int i, const Vec4 v) {
        elements[i * 4] = v.x;
        elements[i * 4 + 1] = v.y;
        elements[i * 4 + 2] = v.z;
        elements[i * 4 + 3] = v.w;
    ...
    void tabulate(const FT* f) {
        for (int i = 0; i < size; i++)
            set(i, f->apply(i));
    }

    template<typename FT>
    void map(const FT* const f, Vec4Array* const src) {
        for (int i = 0; i < size; i++)
            set(i, f->apply(src, i, src->get(i), src->getW(i)));
    }

    template<typename FT>
    Vec3 sum(const FT* const f) {
        Vec3 v(0, 0, 0);
        for (int i = 0; i < size; i++) {
...
public:
    ...
    TVIRTUAL Vec3 apply(Vec4Array* pos, int i, const Vec3 pj, float wj) const {
        Vec3 r = pi.sub(pj);
...
public:
    ...
    TVIRTUAL Vec3 apply(Vec4Array* pos, int i, const Vec3 pi, const float w) const {
...
public:
    TVIRTUAL Vec3 apply(Vec4Array* pos, int i, const Vec3 v, const float w) const {
